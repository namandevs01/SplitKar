...
import { Users, Plus, X, ArrowRight, Home, Plane, Utensils, PartyPopper, Briefcase, Tag } from 'lucide-react';
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {groups.map((g) => {
            ...
            return (
              <Link key={g.id} to={`/app/groups/${g.id}`}
                className="card p-5 hover:border-brand-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-brand-500/5 group">
                <div className="flex items-start justify-between mb-4">
                  <div className="w-12 h-12 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center text-brand-400">
                    <Cat.icon size={22} />
                  </div>
                  <span className="badge-green text-xs">{Cat.label}</span>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-slate-500">
                  <span className="flex items-center gap-1"><Users size={12} />{g.members?.length || 1} members</span>
          ...
          <button onClick={() => setShowModal(true)}
            className="card p-5 border-dashed border-white/10 hover:border-brand-500/30 flex flex-col items-center justify-center gap-3 text-slate-500 hover:text-brand-400 transition-all min-h-[9rem]">
            <Plus size={24} />
